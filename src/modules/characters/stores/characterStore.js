...
export const useCharacterStore = defineStore('characters', () => {
  ...
  // ── Calcul des props dérivées pour un personnage quelconque ──

  /**
   * Calcule toutes les données dérivées nécessaires pour afficher
   * un CharacterSheet pour n'importe quel personnage (pas seulement le sélectionné).
   * @param {Object} char - Personnage
   * @returns {Object} Données dérivées (classData, thresholds, etc.)
   */
  function computePropsForCharacter(char) {
    if (!char) return null
    const cls = resolveClass(char.classId)
    ...
    // Évasion
    const effectiveEvasion = char.evasion + (char.evasionBonus || 0) + bonuses.evasion

    // Score armure
    let effectiveArmorScore
    if (bonuses.armorScoreOverride !== null && bonuses.armorScoreOverride !== undefined) {
      effectiveArmorScore = bonuses.armorScoreOverride + bonuses.armorScore
    } else {
      effectiveArmorScore = char.armorScore + bonuses.armorScore
    }

    // Sous-classes
    let subclasses = []
    const srdSubclasses = getSubclassesForClass(char.classId)
    if (srdSubclasses.length > 0) {
      subclasses = srdSubclasses
    } else {
      const hb = classHomebrewStore.items.find((c) => c.id === char.classId)
      if (hb && Array.isArray(hb.subclasses)) {
        subclasses = hb.subclasses.map((sub, i) => ({
          id: sub.name ? sub.name.toLowerCase().replace(/\s+/g, '-') : `sub-${i}`,
          name: sub.name || `Sous-classe ${i + 1}`,
          spellcastTrait: sub.spellcastTrait || null,
          description: sub.description || '',
          domainOverride: Array.isArray(sub.domainOverride) && sub.domainOverride.length > 0 ? sub.domainOverride : null,
          foundation: Array.isArray(sub.foundation) ? sub.foundation : [],
          specialization: Array.isArray(sub.specialization) ? sub.specialization : [],
          mastery: Array.isArray(sub.mastery) ? sub.mastery : []
        }))
    ...
    if (char.subclassId) {
      subclassData = getSubclassById(char.classId, char.subclassId)
        || subclasses.find((s) => s.id === char.subclassId)
        || null
    }

    // Ascendance (avec support Mixed Ancestry)
    let ancestryData = null
    if (char.ancestryId) {
      if (char.ancestryId === 'mixed-ancestry') {
        const config = char.mixedAncestryConfig
        if (!config) {
          ancestryData = getAncestryById('mixed-ancestry')
        } else {
          const a1 = config.ancestry1Id ? getAncestryById(config.ancestry1Id) : null
          const a2 = config.ancestry2Id ? getAncestryById(config.ancestry2Id) : null
          const base = getAncestryById('mixed-ancestry')
          ...
          ancestryData = {
            ...base,
            name: names.length > 0 ? `Mixed (${names.join(' / ')})` : 'Mixed Ancestry',
            topFeature: feature1,
            bottomFeature: feature2,
            _resolved: true,
            _ancestry1: a1,
        ...
        ancestryData = getAncestryById(char.ancestryId)
      }
    }

    // Communauté
    const communityData = char.communityId
      ? (getCommunityById(char.communityId) || allCommunities.value.find((c) => c.id === char.communityId) || null)
      : null

    // Domaines disponibles
    let charDomains = []
    const srdDomains = getDomainsForClass(char.className)
    if (srdDomains.length > 0) {
      charDomains = srdDomains
    } else if (subclassData && subclassData.domainOverride) {
      charDomains = subclassData.domainOverride.map((dName) => resolveDomainByName(dName)).filter(Boolean)
    } else if (cls && cls.domains) {
      charDomains = cls.domains.map((dName) => resolveDomainByName(dName)).filter(Boolean)
    }

    // Cartes de domaine disponibles
    const charDomainCards = []
    for (const domain of charDomains) {
      for (const card of domain.cards) {
        if (card.level <= char.level) {
          charDomainCards.push({ ...card, domainId: domain.id, domainName: domain.name, domainColor: domain.color, domainEmoji: domain.emoji })
        }
      }
    }

    // Résolution d'une carte (locale)
    function _resolveCard(cardId) {
      const srd = getCardById(cardId)
      if (srd) return srd
      for (const domain of charDomains) {
        const card = domain.cards.find((c) => c.id === cardId)
      ...
      for (const hb of domainHomebrewStore.items) {
        const card = (hb.cards || []).find((c) => c.id === cardId)
        if (card) return { ...card, domain: hb.id }
      }
      return null
    }

    function _resolveMeta(domainId) {
      const domain = charDomains.find((d) => d.id === domainId)
        || domainHomebrewStore.items.find((d) => d.id === domainId)
      return {
        domainColor: domain?.color || '#53a8b6',
        domainName: domain?.name || '',
    ...
    // Loadout + vault cards
    const loadoutCards = (char.domainCards?.loadout || [])
      .map((cardId) => {
        const data = _resolveCard(cardId)
        if (!data) return null
        return { ...data, ..._resolveMeta(data.domain) }
      })
      .filter(Boolean)

    const vaultCards = (char.domainCards?.vault || [])
      .map((cardId) => {
        const data = _resolveCard(cardId)
        if (!data) return null
        return { ...data, ..._resolveMeta(data.domain) }
    ...
    // Max loadout
    let bonusFromAdvancements = 0
    if (Array.isArray(char.levelHistory)) {
      for (const entry of char.levelHistory) {
        if (!Array.isArray(entry.advancements)) continue
        for (const adv of entry.advancements) {
          if (adv.type === 'domain_card') bonusFromAdvancements++
        }
      }
    }
    let bonusFromSubclass = 0
    if (char.subclassId) {
      const sub = getSubclassById(char.classId, char.subclassId)
      if (sub && sub.domainCardBonuses) {
        const prog = char.subclassProgression || 'foundation'
        bonusFromSubclass = sub.domainCardBonuses.foundation || 0
        if (prog === 'specialization' || prog === 'mastery') bonusFromSubclass += sub.domainCardBonuses.specialization || 0
        if (prog === 'mastery') bonusFromSubclass += sub.domainCardBonuses.mastery || 0
      }
    }
    const maxLoadout = getMaxLoadout(char.level, bonusFromAdvancements + bonusFromSubclass)
    const isLoadoutFull = (char.domainCards?.loadout?.length || 0) >= maxLoadout
    ...
    for (const cardId of loadout) {
      ...
      let statusLabel = ''
      switch (mod.type) {
        case 'passive': active = true; statusLabel = 'Actif'; break
        case 'conditional':
          active = typeof mod.isActive === 'function' && mod.isActive(char)
          statusLabel = active ? mod.conditionLabel : `Inactif (${mod.conditionLabel})`
          break
        ...
        case 'toggle':
          canToggle = true; active = !!activeEffects[cardId]
          statusLabel = active ? mod.toggleLabel : `Inactif (${mod.toggleLabel})`
          break
        case 'activable':
          canToggle = true; active = !!activeEffects[cardId]
          statusLabel = active ? 'Activé' : 'Disponible'
          break
        case 'permanent': continue
        default: break
      }
      activeModifiers.push({
        cardId, name: mod.name, domain: mod.domain, icon: mod.icon,
        type: mod.type, description: mod.description, active, canToggle,
        statusLabel, frequency: mod.frequency || null, cost: mod.cost || null,
        toggleLabel: mod.toggleLabel || null
      })
    }
    ...
    return {
      classData: cls,
      statBonuses: bonuses,
      effectiveMaxHP: baseHP + bonuses.maxHP,
      effectiveMaxStress: baseStress + bonuses.maxStress,
      thresholds,
      effectiveEvasion,
      effectiveArmorScore,
      subclasses,
      ...
      ancestryData,
      communityData,
      availableDomains: charDomains,
      availableDomainCards: charDomainCards,
      loadoutCards,
      vaultCards,
      maxLoadout,
      isLoadoutFull,
      activeModifiers,
      permanentEffects: char.permanentCardEffects || []
    }
  }
  ...
  return {
    ...
    // Calcul props par personnage
    computePropsForCharacter,
