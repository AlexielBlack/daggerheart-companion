<template>
  ...
  >
    ...
    <div class="pc-card__body">
      <div class="pc-card__identity">
        <button
          ...
          @click.stop="$emit('toggle-hidden', pc.id)"
        >
          &#x1F441;&#xFE0F;
        </button>
        <span
    ...
    <div
      v-if="currentVisibleTabs.length > 0"
      class="pc-card__tabs"
    >
      <div
      ...
      >
        <button
          v-for="tab in currentVisibleTabs"
          :id="'tab-' + pc.id + '-' + tab.id"
          :key="tab.id"
          class="pc-card__tab"
          :class="{ 'pc-card__tab--active': activeTab === tab.id }"
          role="tab"
...
<script>
/**
 * PcCard — Carte individuelle d'un personnage joueur.
 * Affiche identite, traits, onglets (Capacites/Domaines/Inventaire/Notes),
...
export default {
  ...
  setup(props) {
    ...
    // ── Onglet actif local a cette carte ──
    const activeTab = ref('capacites')
    ...
    // Focus automatique sur l'input d'edition
    ...
    // Onglets visibles pour le PJ courant (computed)
    const currentVisibleTabs = computed(() => visibleTabs(props.pc))

    function navigateTab(direction) {
      const tabs = currentVisibleTabs.value
      const currentIdx = tabs.findIndex(t => t.id === activeTab.value)
      const nextIdx = (currentIdx + direction + tabs.length) % tabs.length
      activeTab.value = tabs[nextIdx].id
      nextTick(() => {
        const el = document.getElementById('tab-' + props.pc.id + '-' + tabs[nextIdx].id)
        if (el) el.focus()
      })
    }
