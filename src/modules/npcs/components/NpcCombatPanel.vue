  ...
  <fieldset class="npc-combat">
    ...
    <template v-if="combatProfileMode === 'custom'">
      ...
      <HomebrewFeatureForm
        ref="homebrewFormRef"
        :themes="themes"
        :combat-features="combatFeatures"
        @feature-deleted="onHomebrewDeleted"
      />
    </template>
  </fieldset>
</template>
...
import HomebrewFeatureForm from './HomebrewFeatureForm.vue'
import {
...
export default {
  name: 'NpcCombatPanel',
  components: { HomebrewFeatureForm },
  ...
  setup(props, { emit }) {
    ...
    // ── Ref vers le formulaire homebrew (expose startEdit, deleteFeature) ──
    const homebrewFormRef = ref(null)

    function editHomebrew(id) {
      if (homebrewFormRef.value) homebrewFormRef.value.startEdit(id)
    }

    function deleteHomebrew(id) {
      if (homebrewFormRef.value) homebrewFormRef.value.deleteFeature(id)
    }

    function onHomebrewDeleted(id) {
    ...
    return {
      ...
      unlockAllTiers,
      domainOptions,
      // Homebrew (délégué à HomebrewFeatureForm)
      homebrewFormRef,
      editHomebrew,
      deleteHomebrew,
      onHomebrewDeleted,
