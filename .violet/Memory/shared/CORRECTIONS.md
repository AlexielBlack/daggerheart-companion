...
### [VRAIE CAUSE] Création d'adversaire custom : échec de validation avalé silencieusement
**Date:** 2026-06-20 13:10 — Violet
**Error:** La création passe par l'éditeur INLINE de `AdversaryBrowser.vue` (onSaveInline), pas par l'ancien HomebrewAdversaryEditor. Deux défauts : (1) le `<HomebrewForm>` inline recevait `:errors="[]"` codé en dur → aucune erreur de champ affichée, et aucun `submit-error` ; (2) `onSaveInline` appelait `homebrewStore.create(data)` et, si `!result.success`, **fermait l'éditeur sans rien afficher**. Comme le schéma adversaire rend `attack.name` et `attack.damage` REQUIS (sans defaultValue), tout adversaire créé sans nom d'attaque + dégâts échouait à la validation → « rien ne se passe », 0 créé. Reproduit : reset()+name+tier+type → create() = {success:false, errors:[attack.name requis, attack.damage requis]}. Diagnostic confirmé en navigation privée (vrai code déployé, localStorage vide) : impossible de créer le moindre adversaire.
**Correction:** `AdversaryBrowser.vue` — refs `validationErrors`/`submitError` ; `onSaveInline` garde l'éditeur ouvert et affiche les erreurs si `!result.success` (create ET update) ; `<HomebrewForm :errors="validationErrors" :submit-error="submitError">`. L'utilisateur voit désormais « Nom de l'attaque est requis / Dégâts est requis » et peut compléter. 2927 tests verts, build OK.
**Leçon majeure (process):** Tout mon diagnostic initial (persistance async, quota, cache) était FAUX car (a) j'ai tapé la mauvaise clé localStorage (`adversaires` FR au lieu de `adversaries` EN) pendant ~6 allers-retours, (b) je n'ai pas vérifié le VRAI chemin de création (éditeur inline) avant de théoriser. RÈGLE : pour un bug « X ne marche pas », LIRE d'abord le code du chemin exact (qui appelle quoi), reproduire par test, AVANT d'émettre des hypothèses. Et ne jamais écrire dans une clé de prod pour un test. Question ouverte pour 2J : `attack` doit-il rester requis (adversaires sans combat / Social) ?

### Level-up : catalogue de cartes de domaine plafonné au niveau courant (off-by-one)
**Date:** 2026-06-21 09:48 — Violet
**Error:** Dans l'assistant de montée de niveau (DomainCardStep), on ne pouvait choisir que des cartes de niveau STRICTEMENT inférieur au nouveau niveau. Cause racine : `LevelUpWizard.catalogCards` consommait `charStore.availableDomainCards`, getter plafonné à `card.level <= char.level` (niveau COURANT). Or la montée n'est appliquée qu'au `finalize()` : pendant l'assistant, `char.level` est encore l'ancien niveau, donc les cartes du nouveau niveau (level = targetLevel) étaient exclues du catalogue. Le filtre « Niveau max » (≤ targetLevel) et `levelOptions` étaient corrects — c'était la SOURCE des données qui était tronquée.
**Correction:** characterStore — extraction d'un helper `domainCardsUpToLevel(maxLevel)` (même shape avec domainId/domainName/etc.) ; `availableDomainCards` l'appelle avec `char.level` (inchangé pour la feuille / jeu normal). `LevelUpWizard.catalogCards` appelle désormais `charStore.domainCardsUpToLevel(store.targetLevel)`. Le picker de la feuille reste plafonné au niveau courant (correct hors level-up). 3 tests de régression dans domainCards.test.js. 295 tests characters verts, build OK.
**Leçon:** Un getter « disponible » partagé entre deux contextes (jeu normal vs level-up) a deux plafonds différents. Paramétrer par niveau plutôt que recoder le filtre dans le composant.

### Combat live : attaque standard de l'adversaire invisible dans le panneau de détail
**Date:** 2026-06-21 12:22 — Violet
**Error:** En combat (EncounterLive → ContextPanel), sélectionner un adversaire n'affichait aucune capacité offensive de base. Le `ContextPanel` (panneau de détail adversaire) ne rendait QUE les expériences, motives et features classifiées (passives/actions/réactions) — jamais l'objet `attack` (attaque standard). Particulièrement visible pour les adversaires custom (homebrew), dont l'offensive est souvent encodée uniquement dans le champ « Attaque standard » (optionnel depuis 9114c723). Effet aggravant : les features de type `action` sont volontairement masquées hors tour MJ (`classifyAdversaryFeatures` ne les renvoie que si `sceneMode === 'adversaryAttack'`) — donc en mode normal, un adversaire custom sans feature passive/réaction n'affichait STRICTEMENT rien d'offensif. Le composant `AdversaryTargetPanel` affichait bien l'attaque (`v-if="isActor && adversary.attack"`) mais il n'est rendu nulle part dans l'app.
**Correction:** ContextPanel.vue — ajout d'un computed `advAttack` (= `adversary.attack` si `modifier` défini) et d'un bloc d'affichage `.ctx-panel__attack` (icône phy/mag, nom, modificateur, dégâts, portée) placé après motives/expériences et avant les features, toujours visible quel que soit le mode de scène. CSS calqué sur `.adv-panel__attack`. ESLint clean, 571 tests encounter verts, build OK.
**Leçon:** Quand un même bloc d'info existe dans deux composants (AdversaryTargetPanel vs ContextPanel) mais qu'un seul est réellement monté, vérifier lequel est dans l'arbre de rendu. L'attaque standard est une capacité offensive « toujours active » et ne doit pas suivre le gating tour-MJ des features `action`.

### Suite — actions adversaire consultables en permanence dans le ContextPanel
**Date:** 2026-06-21 12:24 — Violet
**Demande 2J:** les features de type `action` doivent être consultables en permanence (pas seulement au tour MJ).
**Correction:** on NE touche PAS à `classifyAdversaryFeatures` (son gating tour-MJ est volontaire et couvert par des tests — flux projecteur). Dans `ContextPanel.vue`, `advClassified` appelle désormais `classifyAdversaryFeatures(adversary, SCENE_MODE_ADVERSARY_ATTACK)` en forçant le mode « tour MJ » : comme la fonction ne masque QUE `actionFeatures` (passives/réactions renvoyées indépendamment du mode), les actions sont toujours peuplées dans ce panneau de consultation, sans impacter les autres usages. ESLint clean, 571 tests encounter verts, build OK.
**Leçon:** Pour changer l'affichage d'un seul consommateur d'une fonction partagée et testée, paramétrer l'appel côté consommateur plutôt que muter la fonction.
